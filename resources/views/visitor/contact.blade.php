...
@if(Session::has('success'))
    ...
    <section id="success-message" class="text-gray-600 body-font relative">
        <div class="container px-5 lg:px-60 pt-24 mx-auto text-center">
            <h5 class="text-4xl text-indigo-700 font-semibold">Thanks for being awesome!</h5>
            <br>
            <p class="text-lg leading-relaxed">
                ...
                <br><br>
                Talk to you soon, <span class="font-semibold">Nanovity</span>.
            </p>
        </div>
    </section>
        ...
        <div class="lg:w-1/3 md:w-1/2 bg-white flex flex-col md:ml-auto w-full md:py-8 mt-8 md:mt-0">
            ...
            <form method="POST" action="{{ route('contact.store') }}">
                ...
                <div class="relative mb-4">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" class="w-full bg-white rounded border border-gray-300 focus:border-indigo-600 @error('name') border-red-500 @enderror focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                    @error('name')
                ...
                <div class="relative mb-4">
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email') }}" class="w-full bg-white rounded border border-gray-300 focus:border-indigo-600 @error('email') border-red-500 @enderror focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                    @error('email')
                        <p class="text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <div class="relative mb-4">
                    ...
                    <textarea id="message" name="message" class="w-full bg-white rounded border border-gray-300 focus:border-indigo-600 @error('message') border-red-500 @enderror focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out">{{ old('message') }}</textarea>
                    @error('message')
                        <p class="text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <button type="submit" class="text-white bg-indigo-700 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg">Send Email</button>
            </form>
        </div>
    </div>
</section>
